...
public class MainActivity extends AppCompatActivity {
    ...
    private final float X_Cat = 300; // задание поля для координаты X
    private final float Y_Cat = 800; // задание поля для координаты Y
        ...
        public boolean onTouch(View view, MotionEvent motionEvent) { // в motionEvent пишутся координаты
            ...
            switch (motionEvent.getAction()) { // метод getAction() считывает состояние касания (нажатие/движение/отпускание)
                ...
                case MotionEvent.ACTION_MOVE: // движение
                    ...
                    if (x < (Y_Cat + deltaCat) && x > (X_Cat - deltaCat) && y < (Y_Cat + deltaCat) && y > (Y_Cat - deltaCat)) { // если пользователь коснулся места нахождения кота
                        ...
                        toast.setGravity(Gravity.LEFT, (int) (X_Cat - 200),(int) (Y_Cat - 300)); // задание позиции на экране (положение, смещение по оси Х, смещение по оси Y)
